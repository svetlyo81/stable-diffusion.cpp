...
bool ModelLoader::save_to_gguf_file(const std::string& file_path, ggml_type type, const std::string& tensor_type_rules_str, const std::unordered_map<std::string, float>& loras) {
...
bool convert(const char* input_path, const char* vae_path, const char* output_path, sd_type_t output_type, const char* tensor_type_rules, const char* prompt, const char* lora_model_dir) {
    ...
    bool success = model_loader.save_to_gguf_file(output_path, (ggml_type)output_type, tensor_type_rules, loras);
    return success;
}
