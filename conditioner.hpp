...
struct FrozenCLIPEmbedderWithCustomWords : public Conditioner {
    ...
    bool did_read_new_embeddings = false;
    ...
    std::pair<std::vector<int>, std::vector<float>> tokenize(std::string text,
        ...
        auto on_new_token_cb = [&](std::string& str, std::vector<int32_t>& bpe_tokens) -> bool {
            ...
            embd_name             = trim(embd_name);
            if (embd_name.size() > 0 &&
                embd_name.find(".safetensors") != std::string::npos) {
                if (load_embedding(embd_name, embd_name, bpe_tokens)) {
                    if (word_end != std::string::npos) {
                        str = str.substr(word_end);
                    } else {
                        str = "";
    ...
    SDCondition get_learned_condition(ggml_context* work_ctx,
        ...
        if (did_read_new_embeddings) {
            tokens_and_weights = tokenize(text, true);
            did_read_new_embeddings = false;
        }
