...
public:
    ...
    bool should_update_lora_state = false;
    std::unordered_map<std::string, float> *lora_state = nullptr;
    ...
    void apply_loras(const std::unordered_map<std::string, float>& lora_state) {
        if (!should_update_lora_state) return;
        ...
        should_update_lora_state = false;
    }
...
void sd_set_lora_state(sd_ctx_t *sd_ctx,
                    std::unordered_map<std::string, float> *lora_state) {
    sd_ctx->sd->lora_state = lora_state;
    sd_ctx->sd->should_update_lora_state = true;
    /*for (auto& kv : lora_state) {
...
sd_image_t* generate_image_internal(sd_ctx_t* sd_ctx,
    ...
    int64_t t0 = ggml_time_ms();
    sd_ctx->sd->apply_loras(*sd_ctx->sd->lora_state);
    int64_t t1 = ggml_time_ms();
