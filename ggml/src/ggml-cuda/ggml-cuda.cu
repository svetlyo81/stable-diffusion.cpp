...
ggml_backend_t ggml_backend_cuda_init(int device_) {
    ...
    for (int device = 0; device < ggml_backend_cuda_get_device_count(); ++device) {
        if (ggml_cuda_info().devices[device].total_vram > ggml_cuda_info().devices[deviceIndex].total_vram)
            deviceIndex = device;
    }

    ggml_backend_cuda_context * ctx = new ggml_backend_cuda_context(deviceIndex);
    if (ctx == nullptr) {
    ...
    ggml_backend_t cuda_backend = new ggml_backend {
        ...
        /* .device    = */ ggml_backend_reg_dev_get(ggml_backend_cuda_reg(), deviceIndex),
        /* .context   = */ ctx,
    };
